...
public partial class Detail : System.Web.UI.Page
{
	...
	protected void DetailsView2_ItemUpdating(object sender, DetailsViewUpdateEventArgs e)
	{
		...
		if (string.IsNullOrWhiteSpace(message))
		{
			var previousStatusLabel = (Label)DetailsView2.FindControl("LblPreviousStatus");
			var previousStatus = !string.IsNullOrWhiteSpace(previousStatusLabel.Text) ? int.Parse(previousStatusLabel.Text) : (int?)null;

			// Send email if project has changed status from spec to live
			if (
				previousStatus != null &&
				previousStatus == (int)Constants.Status.Spec &&
				project.Status == (int)Constants.Status.Live)
			{
				string[] subjectParameters = { project.Code };
